...
function showNum() {
    const btnIban = document.querySelector(".btn-show-iban");
    if (btnIban === null) return false;
    const iban = document.querySelector(".num");
    btnIban.addEventListener("click", () => {
        iban.classList.toggle("showNum");
    });
}

function redirection() {
    let urlCuiurrent = window.location;
    if (urlCuiurrent.pathname == '/gracias.html') {
        setTimeout(() => {
            window.location.href = 'https://bodarobertoycrista.com/'
        }, 15000);
    }
}
